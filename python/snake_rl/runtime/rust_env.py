...
try:
    import snake_core_py  # type: ignore
...
class _PythonBatchEnv:
    ...
    def _obs(self) -> np.ndarray:
        obs = np.zeros((self.config.num_envs, 7, self.config.board_h, self.config.board_w), dtype=np.float32)
        for i in range(self.config.num_envs):
